...
"""
    plot_MNIST_digit(digit; inset=:nothing, subplot=:nothing)

Plots a single MNIST digit as a 28x28 grayscale heatmap. The `digit` argument is a
784-element vector (or 28x28 matrix) of pixel intensities. When `inset` and `subplot`
are provided, the digit is rendered as an inset subplot within an existing plot.
"""
...
"""
    plot_MNIST_grid(; idx1=1, idx2=4, N=10)

Plots an N x N grid of linear combinations `α₁·x₁ + α₂·x₂` of two MNIST training
images (indexed by `idx1` and `idx2`) as inset heatmaps, with axes showing the mixing
...
"""
    plot_MNIST_SVD(; N=1000, k=5, idx=2, base_filename="~/Downloads/mnist_svd")

Computes the SVD of the first `N` MNIST training images and visualizes the top `k`
principal basis images. Also displays the original image at index `idx` alongside its
rank-`k` reconstruction, saving all plots as SVG files to `base_filename`.
"""
...
"""
    compute_MNIST_rotation_matrix(; α=π/2)

Computes a 784x784 matrix that rotates the content of a 28x28 MNIST digit image by
angle `α` (in radians) around the image center using bilinear interpolation. The
returned matrix `A` can be applied to a vectorized image via `A * img`.
"""
...
"""
    plot_MNIST_rotation_matrix(; idx=5, base_filename="~/Downloads/")

Generates a suite of rotation visualizations for the MNIST training image at index `idx`.
Displays the original image, rotations at +/- π/16 radians, heatmaps of the corresponding
rotation matrices, and an animated MP4 movie of the image rotating through a full 2π cycle.
All outputs are saved to the directory specified by `base_filename`.
"""
