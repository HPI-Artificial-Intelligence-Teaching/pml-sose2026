...
function plot_box_mueller(n; α = 0.1, base_name)
    ...
    for i in axes(X, 2)
        X[:, i] = [
            sqrt(-2 * log(X[1, i])) * cos(2 * π * X[2, i])
            sqrt(-2 * log(X[1, i])) * sin(2 * π * X[2, i])
        ]
    end
    p = plot(
        X[1, :],
...
```
"""
function plot_normal_with_marginal(
    μ,
    Σ;
    x2 = [0.0],
    x1_lim = (-3, 3),
...
function main()
    ...
    plot_normal_bayes(
        ...
        x1 = [-2, -1, 0, 1, 2],
        α = 0,
        file_name = "~/Downloads/bayes1.png",
    )
    plot_normal_bayes(
        0,
        1,
        ...
        x1 = [-2, -1, 0, 1, 2],
        α = 0.1,
        file_name = "~/Downloads/bayes2.png",
    )
    plot_normal_bayes(
        0,
        1,
        ...
        x2 = [-2, -1, 0, 1, 2, 3],
        α = 0.1,
        file_name = "~/Downloads/bayes3.png",
    )
end
