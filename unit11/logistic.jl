...
Learns a logistic regression model for `n` examples of the MNIST examples of the digits with the label `class0` and `class1` and outputs the learned weight vector as an image (using `ϵ` as a stopping criterion)
"""
function learn_logistic_regression(n = 250; class0 = 1, class1 = 9, no_iterations = 3)
    # plot the raw data
    y = MNIST(split = :train).targets
    X = MNIST(split = :train).features
    idx0 = range(1, length(y))[y.==class0]
