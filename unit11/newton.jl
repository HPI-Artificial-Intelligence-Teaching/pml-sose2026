...
module NewtonRaphsonOpt

using LaTeXStrings
using Plots

"""
    plot_newton_raphson(f, f′, f′′, x_ts)

Plots the function `f` and its first derivative `f′`, then visualizes the Newton-Raphson
optimization steps starting from each initial point in `x_ts`. At each step, the linear
approximation to the first derivative is shown along with the update direction.
"""
