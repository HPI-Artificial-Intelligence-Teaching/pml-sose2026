...
"""
    plot_newton_raphson(f, f′, f′′, x_ts; xs=range(start=-1.5, stop=2, length=100))

Plots the repeated application of the Newton-Raphson optimization algorithm. Given a function
`f`, its first derivative `f′`, second derivative `f′′`, and a vector of starting points
`x_ts`, this visualizes the iterative update steps. For each starting point, two Newton-Raphson
iterations are performed, showing the tangent line approximation to the derivative and the
resulting update direction. The range `xs` controls the x-axis domain for plotting.
"""
